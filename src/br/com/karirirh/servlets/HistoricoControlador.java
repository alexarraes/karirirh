	...
	protected void service(HttpServletRequest request,
		...
		if (request.getSession().getAttribute("user") == null) {
			...
			if (acao != null && acao.equals("menuHistorico")) {
				...
				List<HistoricoSalario> hSalarios = hSalarioDAO.pesquisarEq(
						"colaborador", col);
				List<HistoricoSetor> hSetores = hSetorDAO.pesquisarEq(
						"colaborador", col);
				List<Ferias> ferias = feriasDAO.pesquisarEq("colaborador", col);
				List<HistoricoAfastamento> hAfastamento = hAfastamentoDAO
						.pesquisarEq("colaborador", col);
				List<MedidaDisciplinar> mD = mDDAO.pesquisarEq("colaborador",
						col);
